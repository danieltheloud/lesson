...
이 예제에서 `test2 = test1`은 test2가 test1이 참조하고 있는 객체를 참조하도록 만듭니다. 따라서 이후에 test2의 값을 변경하면 실제로는 test1의 객체가 변경되었습니다.

Java에서는 객체의 복사가 아닌 참조(reference)를 다루므로 이러한 동작이 발생합니다. 이는 객체의 크기가 큰 경우 메모리 관리 및 성능 측면에서 효율적일 수 있습니다.
